...
from .forms import (
    GradoForm, AlumnoForm, PagoForm, ConsumoForm, FormaPagoForm,
    RegistrarConsumoForm, RegistrarPagoForm, ParametroForm,
)
import requests
import openpyxl
import io
...
def is_operador(user):
    return user.groups.filter(name='operador').exists() or is_admin(user)


def _get_city_from_ip(ip):
    """Determina la ciudad por IP con caché de 1 hora."""
    cache_key = f"geo_city_{ip}"
    ...
    if city is None:
        try:
            ...
            city = geo_data.get('city', '') if geo_data.get('status') == 'success' else ''
        except Exception:
            city = ''
...
def cuentas_corrientes(request):
    ...
    cuentas = cuentas_qs.order_by('alumno__nombre')
...
def index(request):
    ...
    city = _get_city_from_ip(ip) if ip else None
...
def deuda_general(request):
    ...
    cuentas_con_deuda = CuentaCorriente.objects.annotate(
        saldo_annotate=F('total_consumido') - F('total_pagado')
    ...
    for c in cuentas_con_deuda:
        ...
        deudas.append({"alumno": c.alumno, "saldo": c.saldo_annotate, "texto": texto})
